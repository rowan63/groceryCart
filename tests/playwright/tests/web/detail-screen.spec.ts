...
test.describe("DETAIL SCREEN", () => {
  ...
  test(
    "Detail page shows product info, category and add to cart button",
    { tag: "@a1" },
    async ({ page }) => {
      await page.goto("/");
      await page.locator('[data-test-id^="product-"]').first().click();
  ...
  test(
    "Sale item displays on special on detail page",
    { tag: "@a1" },
    async ({ page }) => {
      await page.goto("/");
      await page.locator('[data-test-id^="special-"]').first().click();
      await expect(page).toHaveURL(/\/product\/\d+/);
      await expect(page.getByText("ON SPECIAL")).toBeVisible();
    },
  );
